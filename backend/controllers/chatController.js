...
const getChatsById = asyncHandler(async(req, res) => {
    ...
    if(!isValid) {
        res.status(400).json({message:"Chat does not exist or You don't have permission to acces it."})
    }

    var chats = await Chat.findOne({_id: chatId, users: {$elemMatch:{$eq: userId}}}).populate("users")
    ...
    if(chats === null) {
        res.status(404).json({message:"Chat does not exist or You don't have permission to acces it."})
    } else {
        res.status(200).json(chats)
    }
...
const updateChatName = asyncHandler(async(req, res) => {
    const chatName = await Chat.findByIdAndUpdate(req.params.id, {chatName: req.body.name}, {new: true})
    const chats = chatName && await Chat.findOne({_id: req.params.id, users: {$elemMatch:{$eq: req.user._id}}}).populate("users")
