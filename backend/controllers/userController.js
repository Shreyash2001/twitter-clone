...
const followUnfollowUsers = asyncHandler(async (req, res) => {
  var updated = {};
  const isFollowing =
    req.user.following && req.user.following.includes(req.body.id);
  var options = isFollowing ? "$pull" : "$addToSet";
  await User.findByIdAndUpdate(
    req.user._id,
    { [options]: { following: req.body.id } },
    { new: true }
  );

  updated.user = await User.findByIdAndUpdate(
    req.body.id,
    { [options]: { followers: req.user._id } },
    { new: true }
  );
